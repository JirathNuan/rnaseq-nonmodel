...
Estimated time usage: ~ 15 min per user
...
## Activity
    ...
    ``` bash
    # Current workdir: /home/jiratchaya/Cpa_RNASeq/04_DE_analysis
...
We'll subset the transcriptome assembly using the following command. By extract only the first column of DEG table. The current working directory is `~/Cpa_RNASeq/04_DE_analysis/DESeq2_result`.

``` bash
awk '{print $1}' salmon.isoform.counts.matrix.dark_vs_normal_light.DESeq2.DE_results.*.DE.subset > DEG_seqName.txt
```

Then, go to parent working directory

``` bash
cd ..
```

And extract DEG sequence from assembly file look up by DEG sequence name that we picked up from the previous command.

```bash
retrieve_sequences_from_fasta.pl DESeq2_result/DEG_seqName.txt \
../03_assembly/Trinity_2023-03-08.Trinity.fasta > DEG_sequence.fasta
```

Estimated time usage: ~2 min
